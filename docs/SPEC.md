...
- Test TUI state transitions with Ratatui’s test backend; manually verify resizing, replay, cancellation, and terminal restoration on Windows. Repeat platform-specific verification when Linux/macOS support is added later.
- Run formatting, Clippy, and tests in CI. The current release target is Windows x64 with a checksum and imported-library inspection. Linux x64-musl and macOS Intel/Apple Silicon releases are deferred until native verification is available. Use static linking where supported and inspect resulting dependencies; Rust’s [runtime-linking support is target-dependent](https://doc.rust-lang.org/reference/linkage.html).
